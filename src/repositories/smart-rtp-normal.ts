...
export const stringifySmartNormal: JsonStringifyFunction = fastJson({
  title: 'SmartForNormalRTPController',
  type: 'object',
  properties: {
...
export class SmartForNormalRTPController extends BaseRTPController {
  // 玩家累计返奖金额（所有中奖金额的总和）
  ...
  private killFlag: boolean; // 是否开启杀头

  private lastOrderAmountNumber: number; // 上一局的投注金额
  ...
  static fromJSON(json: any): BaseRTPController {
    const inst = new SmartForNormalRTPController(json.initialDeposit);
  ...
  reportLastResult(
    ...
    console.log('参数:', betAmount, winAmount, currentBalance);
  }
